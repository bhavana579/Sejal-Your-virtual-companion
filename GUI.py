...
def GetAssistantStatus():
    status_file = rf'{TempDirPath}\AIStatus.data'
    
    if not os.path.exists(status_file):
        # Optional: You can create the file with a default value
        with open(status_file, "w", encoding='utf-8') as file:
            file.write("Inactive")  # Or whatever default you want
        return "Inactive"  # Return default status
...
class InitialScreen(QWidget):
    ...
    def toggle_icon(self, event=None):
        if self.toggled:
            ...
            self.load_icon(GraphicsDirectoryPath('Mic_off2.png'), 60, 60) 
            MicButtonClosed() 
